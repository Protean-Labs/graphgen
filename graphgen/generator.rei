...
let generate_directories: unit => result(bool, string);
...
let single_file: string => string => (Subgraph.t => models) => Subgraph.t => result(unit, string);
...
let multi_file: string => ('a => string) => (Subgraph.t => list(('a, models))) => Subgraph.t => result(unit, string);